              ...
              {(!categories || categories.length === 0) ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-[16px] tmd:gap-[24px]">
                  {categories.map((category: Category, index: number) => (
                    <ScrollReveal key={category.id} delay={0.1 * (index % 5)}>
                      <button
                        className="group w-full flex flex-col items-center gap-[10px] bg-white border border-[#E5E7EB] rounded-[16px] p-[16px] hover:shadow-md hover:border-[#8F0024]/60 transition-all duration-200 text-left"
